...
export default function BestDeals() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      data = data.map((p) => ({
        ...p,
        discount: Number(p.oldprice) - Number(p.newprice),
      }));

      // sort by highest discount and take top 5
      const top6 = data.sort((a, b) => b.discount - a.discount).slice(0, 6);
      setProducts(top6);
    };
        ...
        {products.map((product) => (
          <SwiperSlide key={product.id}>
            ...
            >
              ...
              <div className="absolute top-2 left-2 bg-red-600 text-white text-[10px] sm:text-xs font-bold px-1 py-1 rounded-full shadow-md">
                {Math.round(
